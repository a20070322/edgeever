...
test('packed CLI installs independently and supports profiles, HTTP, files and conflicts under Node', async () => {
  ...
  try {
    ...
    assert.equal((await run('--version')).trim(), '0.2.0');
    ...
    server = createServer(async (req, res) => {
      ...
      if (url.pathname === '/api/v1/notebooks') return reply({ notebooks: [{ id: 'nb_test', name: '测试笔记本', parentId: null }] });
    ...
    assert.equal(await readFile(notePath, 'utf8'), memo.contentMarkdown);
    await writeFile(notePath, '# Local edit\n');
    ...
    assert.equal(await readFile(notePath, 'utf8'), '# Local edit\n');
    capable = false;
