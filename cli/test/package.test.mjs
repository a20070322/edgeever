...
test('packed CLI installs independently and supports profiles, HTTP, files and conflicts under Node', async () => {
  ...
  try {
    ...
    assert.equal((await run('--version')).trim(), '0.4.0');
    ...
    server = createServer(async (req, res) => {
      ...
      if (url.pathname === '/api/v1/notebooks') return reply({ notebooks: [{ id: 'nb_test', name: '测试笔记本', parentId: null }, { id: 'nb_other', name: '移动目标', parentId: null }] });
